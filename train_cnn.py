"""
Train EfficientNet-B0 or ResNet-50 for 3-class face authenticity classification.
...
─────
    ...
    python train_cnn.py --model resnet --epochs 25 --batch_size 16
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train CNN backbone.")
    p.add_argument("--model", choices=["efficientnet", "resnet"], default="efficientnet",
                   help="Which backbone to train.")
    p.add_argument("--epochs",        type=int,   default=config.NUM_EPOCHS)
    p.add_argument("--batch_size",    type=int,   default=config.BATCH_SIZE)
    p.add_argument("--lr",            type=float, default=config.LEARNING_RATE)
    p.add_argument("--dropout",       type=float, default=0.3)
    p.add_argument("--unfreeze_after",type=int,   default=config.UNFREEZE_AFTER,
...
def main() -> None:
    ...
    img_size = (
        config.EFFICIENTNET_SIZE if args.model == "efficientnet"
        else config.RESNET_SIZE
    )
    loaders, _ = get_dataloaders(image_size=img_size, batch_size=args.batch_size)

    # ── Build model ────────────────────────────────────────────────────────────
    ModelCls = EfficientNetClassifier if args.model == "efficientnet" else ResNetClassifier
    model    = ModelCls(
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == args.unfreeze_after + 1:
            ...
            scheduler = CosineAnnealingLR(
                optimizer, T_max=args.epochs - args.unfreeze_after
            )

        t0 = time.time()
        tr_loss, tr_acc = train_one_epoch(model, loaders["train"], criterion, optimizer, device)
        vl_loss, vl_acc, _, _, _ = evaluate(model, loaders["val"],  criterion, device)
        scheduler.step()
