"""
ViT-Base baseline for 3-class face authenticity classification.

Architecture choice:
  - Pretrained on ImageNet for strong global visual representations.
  - Uses self-attention to model relationships across image patches.
...
class ViTBaseClassifier(nn.Module):
    def __init__(
        ...
        in_features = self.backbone.num_features  # typically 768
    ...
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        """Return penultimate ViT-Base features (no classification head)."""
